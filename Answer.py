...
class Answer:
    ...
    def get_detail(self):
        ...
        text = None
        try:
            detail = str(soup.find("div", class_ = "zm-editable-content clearfix"))
            text = html2text.html2text(detail)

            r = re.findall(
                r'&lt;img\ssrc=".*"\ndata-rawwidth=".+"\sdata-rawheight=".+"\sclass=".+"\nwidth=".+"&gt;!\[\]\(.+\)',text)
            for i in r:
                t = re.split(r'(&lt;img\ssrc=")(.*)("\ndata-rawwidth=".+"\sdata-rawheight=".+"\sclass=".+"\nwidth=".+"&gt;)(!\[\])\((.+)\)',i)
    ...
    def get_visited_times(self):
        ...
        visited_times = 0
        try:
            visited_times = soup.find("div", class_ = "zm-side-section zh-answer-status").find("div", class_= "zm-side-section-inner").find_all('p')[1].strong.string
        except:
            pass
        finally:
